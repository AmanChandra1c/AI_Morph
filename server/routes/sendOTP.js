...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: "OTP sent successfully", sessionID: req.sessionID });
  } catch (err) {
...
export const verifyOTP = async (otp, sessionID) => {  
  ...
  if (!sessionStore) {
    console.error("Session store not initialized. Call setSessionStore() in your main app.");
