...
const Navbar = () => {
  ...
  const navLinks = [
    { path: "/", label: "Home", show: checkToken },
    { path: "/profile", label: "Profile", show: checkToken },
    { path: "/create-post", label: "Create", show: checkToken },
  ];

  return (
    <nav className="shadow-lg border-b border-gray-200 sticky top-0 z-50 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center space-x-1">
              ...
              (link) =>
                link.show && (
                  <Link
                    key={link.path}
                    to={link.path}
                    className={`relative px-4 py-2 rounded-md font-inter font-medium transition-all duration-300 ease-in-out group text-gray-700}`}
                  >
                    ...
                    <span className="relative">
                      ...
                      <span
                        className={`absolute bottom-0 left-0 w-0 h-0.5 bg-blue-600 transition-all duration-300 group-hover:w-full ${
                          isActive(link.path) ? "bg-green-400" : ""
                        }`}
                      ></span>
                    </span>
                  </Link>
            ...
            {checkToken && (
              <button
                className="relative px-4 py-2 ml-2 text-black font-inter font-medium rounded-md transition-all duration-300 group"
            ...
            <Link
              ...
              className="ml-2 flex items-center border-2 border-gray-700 text-gray-700 px-3 py-2 rounded-md font-medium transition-all duration-300 hover:border-gray-900 hover:shadow-md group"
            >
              <img
                src={github}
                alt="GitHub"
          ...
          <div className="md:hidden flex items-center space-x-2">
            ...
            <Link
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center border-2 border-gray-700 text-gray-700 p-2 rounded-md transition-all duration-300 hover:border-gray-900"
            >
            ...
            {checkToken ? (
              <button
                onClick={toggleMenu}
                className="inline-flex items-center justify-center p-2 rounded-md text-gray-700 hover:text-gray-900 transition-all duration-300"
                aria-expanded="false"
              >
      ...
      {/* Mobile menu panel */}
      <div
        className={`fixed top-0 right-0 h-full w-80 shadow-2xl transform transition-transform duration-300 ease-in-out z-50 md:hidden bg-white backdrop-blur-md ${
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex-1 px-4 py-6 space-y-3">
              ...
              (link) =>
                link.show && (
                  ...
                  >
                    {/* Active indicator - green dot */}
                    {isActive(link.path) && (
                      <span className="absolute left-1 w-2 h-2 bg-green-400 rounded-full animate-pulse"></span>
                    )}
            ...
            {checkToken && (
              <button
                onClick={handleLogOut}
                className="w-full flex items-center px-4 py-3 text-gray-700 font-inter font-medium rounded-lg transition-all duration-300 hover:bg-red-600 hover:shadow-lg"
              >
                Logout
              </button>
            )}
